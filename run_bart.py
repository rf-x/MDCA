...
from dataset import Twitter_Dataset_Bart
from model import MDCABartModel
from trainer_bart import train, evaluate
...
def parse_args():
    ...
    parser.add_argument('--pretrained_model_dir',type=str, default='./pretrained/bart-large-cnn', help='Path to the pretrained model')
    parser.add_argument('--generation_config',type=str, default='generation_config.json', help='File name of generation_config')

    parser.add_argument('--save_model_dir', type=str, default='./checkpoints/MDCA-BART', help='Path to checkpoints')
    ...
    # Model parameters
    parser.add_argument('--hidden_size', type=int, default=1024, help= 'Hidden size of pretrained model.')
    ...
    parser.add_argument("--eval_batch_size", type=int, default=8, help="Batch size for evalating.")
    ...
    parser.add_argument('--eval_metric', type=str, default='a', help='select checkpoint by which metrics')
...
def main():
    ...
    # Bulid Dataset
    train_dataset = Twitter_Dataset_Bart(args, split='train')
    dev_dataset = Twitter_Dataset_Bart(args, split='dev')
    test_dataset = Twitter_Dataset_Bart(args, split='test')

    # Build Model
    model = MDCABartModel(args)
